...
-- These fancy operators were removed in elm 0.16 :(
infixl 4 <~
...
time : Signal Time
time =
  Time.inSeconds <~ Time.every (25 * Time.millisecond)
...
main : Signal Element
main
  = (\f -> collage dimX dimY ((rect dimX dimY |> filled black) :: [f]))
    <~ kaleido 6
...
kaleido n =
 ...
 in
   (\ps t -> group (map (rotate (2*pi*sin t)) ps))
   <~ polys ~ slowTime 3
